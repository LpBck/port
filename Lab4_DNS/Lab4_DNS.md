...
## Описание
Предположим, нам требуется настроить внутренний DNS-сервер в компании `TaksaInc`, которая занимается разработкой программного обеспечения для такс. Ее ресурсы находятся в домене `taksainс.com`. Сеть компании 10.10.0.0/24 разделена на подсети:
...
## Вывод
Все работает так, как и было задумано по схеме 
